...
class SendEmailToolHandler(toolhandler.ToolHandler):
    ...
    def get_tool_description(self) -> Tool:
        return Tool(
            name=self.name,
            description="""Directly sends an email message from Gmail with specified recipient, subject, body, and optional CC recipients.
            
            Do NOT use this tool when you want to send a REPLY to an existing message. For replies, use reply_gmail_email
            with send=True instead.
            """,
            inputSchema={
                "type": "object",
                "properties": {
                    ...
                    "is_html": {
                        "type": "boolean",
                        "description": "If true, the body will be sent as HTML content",
                        "default": False
                    },
                    "cc": {
                        "type": "array",
    ...
    def run_tool(self, args: dict) -> Sequence[TextContent | ImageContent | EmbeddedResource]:
        try:
            ...
            # Send the email with HTML flag if specified
            ...
            if sent_message is None:
                return [
                    TextContent(
                        type="text",
                        text="Failed to send email. Check logs for more details."
                    )
                ]
            ...
            return [
                TextContent(
                    type="text",
                    text=f"Email successfully sent with ID: {sent_message.get('id', 'unknown')}\n\n{json.dumps(sent_message, indent=2)}"
                )
            ]
        
        except Exception as e:
            logging.error(f"Error in SendEmailToolHandler: {str(e)}")
